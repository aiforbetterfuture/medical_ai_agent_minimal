...
- **검증(Verification)**: 생성한 답변이 환자에게 의학적으로 적절하고 안전한지 LLM 기반으로 자체 확인[21]
...
1. **10개 노드 LangGraph 워크플로우**: 상태 기반 순환형 아키텍처로 복잡한 의사결정 처리[22][23][24]
2. **응답 캐시 시스템 (Response Cache)**: 벡터 유사도 기반으로 유사 질의를 감지하여 검증된 응답을 재사용함으로써 30% 질의 처리 감소
3. **능동적 검색 (Active Retrieval)**: 질의 복잡도를 3단계로 분류하여 동적으로 검색 문서 수를 조정함으로써 30% 레이턴시 감소, 40% 비용 절감[25]
4. **하이브리드 검색 (Hybrid Retrieval)**: BM25 키워드 검색과 FAISS 벡터 검색을 RRF로 융합하여 60% 정밀도 향상[26][27][28]
5. **자기개선 메커니즘 (Self-Refine)**: LLM 기반 품질 평가와 동적 질의 재작성으로 50% 품질 향상 (0.52 → 0.78)[29]
6. **이중 안전장치**: 중복 문서 재검색 방지와 품질 진행도 모니터링으로 무한 루프 완전 제거[30]

**목표 3: 정량적 성능 평가 및 검증**

구현한 AI Agent의 정량적 성능을 다각도로 평가하고 검증한다:

- **Synthea 기반 가상 환자**: 80명의 현실적 환자 프로필 (400 멀티턴 대화)[21]
- **정량적 메트릭**: Faithfulness, Answer Relevance, Perplexity[31][32]
- **통계적 검증**: t-test, Cohen's d, p < 0.001 수준 유의성[33][34]
...
- MedCAT2 통합 및 UMLS 기반 의학 엔티티 추출 구현[35][36]
...
**2단계: LangGraph 기반 순환형 아키텍처 설계 (2-3개월)**[22][23][24]

- 10개 전문 노드 구현:
  1. `check_similarity`: 응답 캐시 확인
  2. `classify_intent`: Active Retrieval 의도 분류[25]
  3. `extract_slots`: MedCAT2 슬롯 추출[35]
  4. `store_memory`: 프로필 저장
  5. `assemble_context`: 컨텍스트 조립
  6. `retrieve`: 하이브리드 검색
  7. `generate_answer`: LLM 답변 생성
  8. `refine`: 품질 평가 및 질의 재작성[29]
  9. `quality_check`: 재검색 결정
  10. `store_response`: 응답 캐싱

- 조건부 엣지 및 라우팅 로직 구현
- Self-Refine 순환 구조 및 이중 안전장치 구현[29]

**3단계: Corrective RAG(CRAG) 적용 및 최적화 (1-2개월)**[30][37][38]

- 하이브리드 검색 구현 (BM25 + FAISS + RRF)[26][27][28]
- LLM 기반 품질 평가 시스템 구현
- 동적 질의 재작성 메커니즘 구현
- 성능 최적화 (그래프 캐싱, 설정 캐싱, heapq 최적화, 인덱싱)

**4단계: 가상 환자 데이터 기반 정량적 성능 측정 (1-2개월)**[21]

- Synthea 프레임워크로 80명 가상 환자 생성[21]
- 5턴 멀티턴 시나리오 설계 (총 400턴)
- LLM vs AI Agent 모드 비교 실험
- RAGAS 메트릭 측정 및 통계적 검증[31][33][34]
...
본 연구는 기존 의료 AI 챗봇 및 RAG 시스템과 다음과 같은 차별점을 가진다:[37][38]
...
1. **추출**: 환자의 말에서 중요 증상, 질환, 수치를 파악 → **MedCAT2 + 정규표현식**[35]
...
**MedCAT2 (Medical Concept Annotation Tool 2)**는 UMLS (Unified Medical Language System) 기반 의학 분야 특화 NER(Named Entity Recognition) 모델이다[35][36].

**핵심 기능**:

1. **의학 개념 인식**: 비정형 텍스트에서 의학 용어 자동 감지
2. **UMLS CUI 매핑**: 다양한 표현을 표준 개념 ID로 연결[36]
...
→ 모두 동일한 CUI(C0020538)로 매핑되어 **표준화된 처리** 가능[36]

**구현 코드** (`extraction/slot_extractor.py`):

```python
class SlotExtractor:
    def __init__(self, medcat_model_path: str):[35]
        # MedCAT2 모델 로드[35]
        self.cat = CAT.load_model_pack(medcat_model_path)[35]
        self.confidence_threshold = 0.7  # 신뢰도 임계값
    
    def extract(self, text: str) -> Dict[str, Any]:
        # MedCAT2로 의학 개념 추출[35]
...
MedCAT2가 처리하기 어려운 **인구통계 정보**와 **수치 데이터**는 정규표현식(Regex) 기반 패턴 매칭으로 보완한다[35].
...
1. **우선순위**: 정규표현식 → MedCAT2 (정규표현식이 더 정확한 경우)[35]
2. **보완 관계**: MedCAT2로 개념 추출 + 정규표현식으로 수치 추출[35]
...
- **총 처리 시간**: 50ms 이하 (MedCAT2 30ms + Regex 20ms)[35]
- **정확도**: 인구통계 95%+, 수치 데이터 98%+
- **재현율**: UMLS 커버리지에 따라 70-90%[36]
...
1. **CUI 기반** (MedCAT2 추출): 동일한 CUI면 동일한 개념[35][36]
...
def _dedup_conditions(
    ...
    existing_cuis = {item['cui'] for item in existing if 'cui' in item}[36]
    
    for new_item in new_items:
        new_cui = new_item.get('cui')[36]
        
        if new_cui and new_cui in existing_cuis:[36]
            # 중복: 타임스탬프와 신뢰도 업데이트
            for i, item in enumerate(result):
                if item.get('cui') == new_cui:[36]
...
def _build_evidence_section(retrieved_docs: List[Dict[str, Any]]) -> str:
    ...
    for i, doc in enumerate(retrieved_docs, 1):
        text = doc.get('text', '')
        score = doc.get('rrf_score', doc.get('score', 0))[27]
...
def assemble_context_node(state: AgentState) -> AgentState:
    ...
    # 토큰 예산 관리자 초기화
    budget_mgr = TokenBudgetManager(model_name='gpt-4o-mini')[39]
...
품질 평가 결과의 `missing_info`를 활용하여 **질의를 동적으로 재작성**한다[29].
...
품질 평가와 질의 재작성을 결합하여 **Self-Refine 순환 구조**를 구현한다[29].
...
Self-Refine 과정에서 **무의미한 재검색**과 **무한 루프**를 방지하기 위해 이중 안전장치를 적용한다[29].
...
**LangGraph**는 LangChain이 개발한 상태 기반 워크플로우 프레임워크로, 다음과 같은 특징을 가진다:[22][23][24]
...
**기존 LangChain vs LangGraph**:[22][23][24]
...
본 연구에서 **Self-Refine 순환 구조**가 핵심이므로, 순환 그래프를 지원하는 LangGraph가 필수적이다[22][23][24][29].
...
graph TB
    ...
    ASSEMBLE -->|Active Retrieval 스킵| GEN[generate_answer][25]
...
LangGraph의 핵심 기능인 **조건부 엣지**를 활용하여 동적 라우팅을 구현한다[22][23][24].
...
def _retrieval_router(state: AgentState) -> str:
    ...
    # Active Retrieval이 검색 불필요 판단[25]
    if not needs_retrieval:
        return "generate_answer"
    else:
        return "retrieve"
...
class AgentState(TypedDict):
    ...
    dynamic_k: Optional[int]          # Active Retrieval: 동적 k[25]
...
class ResponseCache:
    def __init__(self, similarity_threshold: float = 0.85):
        ...
        self.vector_index = None  # FAISS 인덱스[28]
    ...
    def store(
        ...
        # FAISS 인덱스 재구축 (batch 업데이트 권장)[28]
        self._rebuild_index()
```
...
def _classify_by_content(query: str) -> Tuple[bool, int, str]:
    ...
    factual_patterns = [
        "뭐", "무엇", "어떻", "왜", "어디", "언제", "누구",
        "what", "how", "why", "where", "when", "who"[40]
    ]
...
**목적**: 키워드 기반 검색(BM25)과 의미 기반 검색(FAISS)을 결합하여 정밀도와 재현율을 모두 향상시킨다[26][28].

##### 3.4.3.1 BM25 키워드 검색

**BM25 (Best Matching 25)**는 TF-IDF를 개선한 확률적 정보 검색 모델이다[26].

**수식**:

```
BM25(D, Q) = Σ IDF(qi) × (f(qi, D) × (k1 + 1)) / (f(qi, D) + k1 × (1 - b + b × |D| / avgdl))[26]
```
...
**구현** (`retrieval/bm25_retriever.py`):[26]

```python
from rank_bm25 import BM25Okapi[26]
import MeCab  # 한글 형태소 분석기

class BM25Retriever:[26]
    def __init__(self, documents: List[str]):
        ...
        # BM25 인덱스 구축[26]
        self.bm25 = BM25Okapi(self.tokenized_corpus)[26]
    ...
    def search(self, query: str, k: int = 8) -> List[Dict[str, Any]]:
        ...
        # BM25 점수 계산[26]
        scores = self.bm25.get_scores(tokenized_query)[26]
        ...
        for idx in top_k_idx:
            if scores[idx] > 0:  # 점수가 0보다 큰 문서만
                results.append({
                    ...
                    'bm25_score': float(scores[idx]),[26]
                    'index': int(idx)
                })
...
**FAISS (Facebook AI Similarity Search)**는 고차원 벡터의 유사도 검색을 최적화한 라이브러리다[28].
...
**구현** (`retrieval/faiss_retriever.py`):[28]

```python
import faiss[28]
import numpy as np

class FAISSRetriever:[28]
    def __init__(self, documents: List[str], embedding_client):
        ...
        self.index = faiss.IndexFlatIP(dimension)  # Inner Product (코사인 유사도)[28]
        
        # L2 정규화 (코사인 유사도를 위해)
        faiss.normalize_L2(self.doc_embeddings)[28]
    ...
    def search(self, query: str, k: int = 8) -> List[Dict[str, Any]]:
        ...
        # L2 정규화
        faiss.normalize_L2(query_vec)[28]
...
→ 키워드 "혈당"이 없어도 의미상 유사한 문서 검색[26]
```

##### 3.4.3.3 RRF (Reciprocal Rank Fusion)

**문제**: BM25와 FAISS의 점수 스케일이 다름[26][28]
- BM25: 0~20 (문서마다 다름)[26]
- FAISS: 0~1 (코사인 유사도)[28]

**해결책**: Reciprocal Rank Fusion으로 순위 기반 결합[27]

**수식**:

```
RRF(d) = Σ 1 / (k + rank_r(d))[27]
```

여기서:
- `d`: 문서
- `r`: 검색 방법 (BM25, FAISS)[26][28]
...
class HybridRetriever:
    def __init__(self, bm25_retriever, faiss_retriever):[26][28]
        self.bm25 = bm25_retriever[26]
        self.faiss = faiss_retriever[28]
        self.k_rrf = 60  # RRF 상수[27]
    
    def search(self, query: str, k: int = 8) -> List[Dict[str, Any]]:
        """하이브리드 검색"""
        
        # BM25 검색[26]
        bm25_results = self.bm25.search(query, k=k*2)[26]
        
        # FAISS 검색[28]
        faiss_results = self.faiss.search(query, k=k*2)[28]
        
        # RRF 융합[27]
        rrf_scores = {}[27]
        
        # BM25 순위 기여[26]
        for rank, result in enumerate(bm25_results, start=1):[26]
            doc_idx = result['index']
            rrf_scores[doc_idx] = rrf_scores.get(doc_idx, 0) + 1 / (self.k_rrf + rank)[27]
        
        # FAISS 순위 기여[28]
        for rank, result in enumerate(faiss_results, start=1):[28]
            doc_idx = result['index']
            rrf_scores[doc_idx] = rrf_scores.get(doc_idx, 0) + 1 / (self.k_rrf + rank)[27]
        ...
        results = []
        for doc_idx, rrf_score in top_k:[27]
            results.append({
                'text': self.bm25.documents[doc_idx],[26]
                'rrf_score': float(rrf_score),[27]
                'index': int(doc_idx)
            })
...
- **상호 보완**: BM25의 정확성 + FAISS의 의미 이해[26][28]
...
**문제**: 매 요청마다 LangGraph 워크플로우를 새로 컴파일하면 100-200ms 오버헤드 발생[22][23][24]
...
**문제**: BM25 점수 계산 시 정렬에 `np.argsort()` 사용 → O(n log n)[26]
...
- `_simple_cache_search()`: FAISS 기반 캐시로 대체[28]
...
본 연구는 기존 의료 AI 챗봇 및 RAG 시스템과 다음과 같은 명확한 차별점을 가진다:[37][38]
...
**2) LangGraph 기반 순환식 아키텍처**[22][23][24]

- **기존**: 일방향 체인 (LangChain) 또는 단순 반복문[22]
- **본 연구**: 10개 전문 노드 + 조건부 엣지로 복잡한 의사결정 구현
- **효과**: Self-Refine 자연스럽게 구현, 디버깅 용이[29]
...
**4) 능동적 검색 (Active Retrieval)**[25]

- **기존**: 모든 질의에 대해 고정된 k개 문서 검색
- **본 연구**: 3단계 분류로 질의 복잡도 판단 → 동적 k 조정[25]
...
**통계적 유의성**:[33][34]
- 모든 메트릭에서 p < 0.001 (t-test)[33][34]
- Cohen's d > 0.8 (Large effect size)[33]
...
2. **Self-Refine의 실용적 구현**[29]
   - 이론적으로만 논의되던 Self-Refine을 LangGraph + LLM Judge로 실제 구현[22][23][24][29]
   - 이중 안전장치로 실용성 확보

3. **Active Retrieval의 의료 도메인 적용**[25]
   - 일반 RAG에서의 Active Retrieval을 의료 도메인의 특성에 맞게 조정[25]
...
6. **다이어그램 6**: MedCAT2 추출 프로세스 플로우차트[35]
7. **다이어그램 7**: MedCAT2 + 정규표현식 결합 구조도[35]
...
15. **다이어그램 15**: Self-Refine 순환 구조 상세 플로우차트[29]
...
22. **다이어그램 22**: Active Retrieval 3단계 분류 플로우차트[25]
23. **다이어그램 23**: 복잡도별 k 값 분포 막대 그래프
24. **다이어그램 24**: 하이브리드 검색 (BM25 + FAISS + RRF) 구조도[26][27][28]
...
- **제2장 (관련 연구)**:
  - [추가 제안 C] RAG 발전 타임라인 (2020-2025)[37][38]
...
- **제4장 (실험 및 결과, 향후 작성)**:
  - [추가 제안 E] 실험 설계 다이어그램
  - [추가 제안 F] Synthea 가상 환자 생성 프로세스[21]
