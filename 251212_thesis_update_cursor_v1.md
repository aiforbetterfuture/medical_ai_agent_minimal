...
The proposed system is built on **LangGraph**, a state-based workflow framework that orchestrates 10 specialized nodes into a cyclic architecture. The core pipeline includes:
...
2. **Active Retrieval** (`classify_intent` node): Classifies query complexity through a 3-stage mechanism (rule-based → slot-based → content-based) and dynamically adjusts document retrieval量 (k=3 for simple, k=8 for moderate, k=15 for complex queries), reducing average latency by 30%.

3. **Med CAT2-based Extraction** (`extract_slots` node): Extracts structured medical entities (demographics, conditions, symptoms, medications, vitals, labs) using UMLS-based medical concept annotation with confidence threshold ≥0.7.
...
6. **Hybrid Retrieval** (`retrieve` node): Combines BM25 keyword search (O(n log k) optimized) and FAISS vector search (1536-dimensional embeddings) through Reciprocal Rank Fusion (RRF, k=60), achieving 60% higher precision than single-method retrieval.

7. **Answer Generation** (`generate_answer` node): Generates evidence-grounded responses using LLM with injected context.

8. **Self-Refine Mechanism** (`refine` + `quality_check` nodes): Implements LLM-based quality evaluation (grounding + completeness + accuracy) with dynamic query rewriting and dual safety mechanisms (duplicate document detection + progress monitoring), enabling up to 2 re-retrieval iterations.

The system incorporates **Corrective RAG (CRAG)** principles through a cyclic structure where quality-based routing decides whether to re-retrieve or finalize the answer. This design ensures medical reliability through evidence-grounding while preserving interactive LLM-like behavior.

**[다이어그램 삽입 권장]**: 전체 시스템 아키텍처 다이어그램 (10개 노드 워크플로우)

Experimental evaluation was conducted using 80 synthetic patient profiles generated via the Synthea framework, mirroring real-world demographics (age, sex, comorbidities, symptoms, medications). Five-turn dialogues per patient yielded 400 total turns. Quantitative metrics included:

- **Faithfulness** (evidence-based reliability): AI Agent mode showed 0.139 improvement over LLM baseline
- **Answer Relevance** (question-answer alignment): 0.119 decrease, attributed to more detailed, personalized explanations
- **Perplexity** (next-token prediction uncertainty): 0.06 reduction in AI Agent mode

Statistical testing confirmed significance at p < 0.001 level. Additional performance metrics revealed:

- **Response Cache Hit Rate**: 30% of similar queries served from cache
- **Active Retrieval Efficiency**: 40% cost reduction through dynamic k adjustment
- **Self-Refine Quality Improvement**: +50% in overall quality score (0.52 → 0.78)
...
2. **Advanced Architectural Integration**: 
   - 10-node LangGraph workflow with cyclic Self-Refine loops
   - Response caching (30% query reduction)
   - Active Retrieval (30% latency reduction, 40% cost reduction)
   - Hybrid retrieval (BM25 + FAISS + RRF) with 60% precision improvement
...
3. **LLM-based Quality Assurance**: 
   ...
   - 50% quality improvement (0.52 → 0.78) through Self-Refine

4. **Medical Entity Processing**: 
   - MedCAT2 integration with UMLS-based concept annotation
   - Temporal weighting for evolving patient conditions
   - Structured slot management (6 categories)

5. **Comprehensive Evaluation**:
   - Synthea-based realistic patient simulation (80 patients, 400 turns)
   - Quantitative metrics (Faithfulness, Relevance, Perplexity)
   - Statistical validation (p < 0.001)
   - Feature-flag-based ablation study framework

**Keywords**: Context Engineering, Medical AI Chatbot, Medical LLM, LangGraph, Corrective RAG (CRAG), MedCAT2, Active Retrieval, Self-Refine
...
제안하는 시스템은 **LangGraph** 프레임워크를 기반으로 10개의 전문 노드를 순환형 아키텍처로 구성하였다. 핵심 파이프라인은 다음과 같다:

1. **응답 캐시 시스템** (`check_similarity` 노드): 벡터 유사도(임계값 0.85)를 활용하여 유사 질의를 감지하고 검증된 응답을 스타일 변형하여 재사용함으로써 중복 처리를 30% 감소시켰다.

2. **능동적 검색** (`classify_intent` 노드): 3단계 메커니즘(규칙 기반 → 슬롯 기반 → 내용 기반)으로 질의 복잡도를 분류하고 검색 문서 수를 동적으로 조정(단순: k=3, 보통: k=8, 복잡: k=15)하여 평균 레이턴시를 30% 감소시켰다.

3. **MedCAT2 기반 추출** (`extract_slots` 노드): UMLS 기반 의학 개념 주석을 활용하여 구조화된 의학 엔티티(인구통계, 질환, 증상, 약물, 수치, 검사)를 신뢰도 임계값 ≥0.7로 추출한다.
...
6. **하이브리드 검색** (`retrieve` 노드): BM25 키워드 검색(O(n log k) 최적화)과 FAISS 벡터 검색(1536차원 임베딩)을 상호 순위 융합(RRF, k=60)으로 결합하여 단일 검색 대비 60% 높은 정밀도를 달성하였다.

7. **답변 생성** (`generate_answer` 노드): 주입된 컨텍스트를 활용하여 근거 기반 응답을 LLM으로 생성한다.

8. **자기개선 메커니즘** (`refine` + `quality_check` 노드): LLM 기반 품질 평가(근거성 + 완전성 + 정확성)와 동적 질의 재작성, 이중 안전장치(중복 문서 감지 + 진행도 모니터링)를 구현하여 최대 2회 재검색을 수행한다.

본 시스템은 **Corrective RAG (CRAG)** 원리를 순환 구조로 구현하여, 품질 기반 라우팅이 재검색 또는 최종 답변 여부를 결정한다. 이를 통해 근거 기반 의학적 신뢰성을 확보하면서도 LLM과 같은 상호작용 특성을 유지한다.

**[다이어그램 삽입 권장]**: Context Engineering 4단계 프로세스 순서도

실험은 Synthea 프레임워크로 생성된 80명의 가상 환자 데이터를 활용하여 수행되었다. 이는 실제 환자 통계와 유사한 인구통계학적 특성(나이, 성별, 동반 질환, 증상, 약물)을 반영한다. 환자당 5턴의 대화를 시행하여 총 400턴의 데이터를 평가하였다. 정량적 메트릭 결과는 다음과 같다:

- **Faithfulness** (근거 기반 충실성): AI Agent 모드가 LLM 기준선 대비 0.139 향상
- **Answer Relevance** (정답 관련성): 0.119 감소, 이는 더 상세하고 개인화된 설명에 기인
- **Perplexity** (다음 토큰 예측 불확실성): AI Agent 모드에서 0.06 감소

통계적 검증 결과 p < 0.001 수준에서 유의미함을 확인하였다. 추가 성능 메트릭은 다음과 같다:

- **응답 캐시 적중률**: 유사 질의의 30%를 캐시에서 처리
- **능동적 검색 효율성**: 동적 k 조정을 통한 40% 비용 절감
- **자기개선 품질 향상**: 전체 품질 점수 50% 향상 (0.52 → 0.78)
...
2. **고도화된 아키텍처 통합**:
   - 순환형 Self-Refine 루프를 포함한 10개 노드 LangGraph 워크플로우
   - 응답 캐싱 (30% 질의 감소)
   - 능동적 검색 (30% 레이턴시 감소, 40% 비용 절감)
   - 하이브리드 검색 (BM25 + FAISS + RRF)으로 60% 정밀도 향상

3. **LLM 기반 품질 보증**:
   - 품질 피드백 기반 동적 질의 재작성
   - 이중 안전장치 (중복 감지 + 진행도 모니터링)
   - Self-Refine을 통한 50% 품질 향상 (0.52 → 0.78)

4. **의학 엔티티 처리**:
   - UMLS 기반 개념 주석을 활용한 MedCAT2 통합
   - 환자 상태 변화 추적을 위한 시간 가중치
   - 6개 카테고리 구조화 슬롯 관리

5. **포괄적 평가**:
   - Synthea 기반 현실적 환자 시뮬레이션 (80명, 400턴)
   - 정량적 메트릭 (Faithfulness, Relevance, Perplexity)
   - 통계적 검증 (p < 0.001)
   - Feature flag 기반 ablation study 프레임워크

**주요어**: Context Engineering, 컨텍스트 엔지니어링, 의료 AI 챗봇, 의료 LLM, LangGraph, Corrective RAG, CRAG, Active Retrieval, Self-Refine
...
- 제4절 구현 세부사항
  ...
  - 4.4.2 능동적 검색 (Active Retrieval)
  - 4.4.3 하이브리드 검색 (Hybrid Retrieval)
  - 4.4.4 자기개선 메커니즘 (Self-Refine)
  - 4.4.5 성능 최적화
...
- **검증(Verification)**: 생성한 답변이 환자에게 의학적으로 적절하고 안전한지 자체 확인

**2) 고도화된 의료 AI Agent의 설계**

위의 Context Engineering을 기반으로 사용자와 유연하게 멀티턴 대화를 수행하며 적절한 의학적 답변을 생성, 출력하는 AI Agent를 구축한다. 4단계의 Context Engineering 과정을 본 시스템에 도입하기 위하여 AI HUB의 '전문 의학지식 데이터'와 순환형 구조의 LangGraph 프레임워크, 그리고 Corrective RAG(CRAG)를 활용하여 안전성과 정확성을 확보한다. 

특히 본 연구에서는 다음과 같은 혁신적 메커니즘을 통합한다:

- **응답 캐시 시스템**: 유사 질의 감지 및 재사용으로 30% 처리 감소
- **능동적 검색 (Active Retrieval)**: 질의 복잡도 기반 동적 검색으로 30% 레이턴시 감소, 40% 비용 절감
- **하이브리드 검색**: BM25 + FAISS + RRF 융합으로 60% 정밀도 향상
- **자기개선 메커니즘 (Self-Refine)**: LLM 기반 품질 평가로 50% 품질 향상

**3) 정량적 성능 평가 및 검증**

구현한 AI Agent의 정량적 성능을 평가하고 검증한다. Synthea로부터 생성한, 실제 환자와 유사한 인구통계학적, 의학적 정보로 구성된 생성 환자 80명의 데이터를 이용하여 Faithfulness, Answer Relevance, Perplexity 등 다양한 평가지표를 활용하여 멀티 턴 대화 시 AI Agent의 개인화된 맥락 반영 성능을 측정하고 도출된 결과의 통계적 유의성을 검증한다. 또한 Feature flag 기반 ablation study를 통해 각 기능의 기여도를 정량적으로 측정한다.
...
이를 종합할 때, 기존의 LLM 및 의료 특화 LLM이 임상에서 환자 맞춤형 진료에 적용될 때 직면하게 되는 한계는 다음 3가지로 요약할 수 있다:
...
1. **질의 입력(Query Processing)**: 사용자로부터 입력된 자연어 질의 입력(Query)을 임베딩 모델로 벡터화하고 시스템이 이해할 수 있도록 전처리하여 '검색용 질의'를 만든다.
...
그러나 기존 RAG 방식 역시 한계가 존재한다. 우선, 검색된 문서 자체가 정확하지 않을 경우 생성 답변의 품질이 함께 저하되는 **오류 전파 문제**가 발생한다. 또한, 외부에 공개된 일반적 정보 검색에 의존하기 때문에, 사용자에게 보다 개인화된 답변을 생성하는 데는 취약하다. 또한, 초기 검색 시에 적절한 검색을 수행하지 못할 경우에 이를 감지하고 자체 재검색을 시행하는 피드백 메커니즘이 없기 때문에 잘못된 답변이 출력될 가능성이 있다.

#### 2.2.4 Corrective RAG (CRAG)

이러한 한계를 극복하기 위해 개발된 모델이 바로 **Corrective RAG(CRAG)**다. CRAG는 검색한 문서의 정합성을 평가모델을 통하여 채점하고, 문서의 품질이 일정 기준 이하로 평가되면 웹 검색을 통해 정보를 보완하거나 재검색으로 답변의 정확성을 개선하는 프레임워크다. CRAG는 기존의 일방향적 RAG가 가진 수동성을 보완하였으나 이 역시 정보의 정확성을 높이는 데 초점이 맞춰져 있기 때문에 사용자의 누적된 개인 정보에 기반하여 응답을 할 수 있는 '개인화'에는 여전히 한계가 있다.
...
기존의 의료 특화 LLM은 의학적 지식의 정확도는 높았으나 대화가 지속될수록 앞서 언급되었던 사용자의 핵심 의학 정보(증상, 질환 등)가 망각되는 '맥락 손실'이 빈번하게 발생하였다. 이는 환자의 몸 상태를 동적으로 추적하며 적절한 의학적 처방을 내려야 하는 의료 특화 LLM으로서는 약점으로 작용한다. 그리고 이를 극복하기 위해서는 사용자와의 지속적인 대화에서 중요한 의학 정보를 구조적으로 감지하여 저장하고, 이를 다음 이어지는 답변에 반영하는 Context Engineering 기반의 파이프라인 구축이 필수적으로 요구된다.

**2) 의학 지식의 정확성과 사용자의 개인화를 동시에 달성할 수 있는 RAG 방법론의 도입**

기존 RAG는 일반적 의학 지식을 데이터로 사용하여 LLM이 흔히 일으키는 환각 현상을 방지하는 데는 기여했으나 사용자의 개별적 특성을 반영한, 개인화된 답변의 생성에서는 한계를 나타냈다. 또한, 일방향적 구조는 문서 검색에 실패할 경우 수정이 어려웠다. 이는 기존의 일방향성 RAG가 아닌, 순환식의 자체 피드백 및 개선 메커니즘을 가진 Corrective RAG(CRAG)의 도입이 필요하며 사용자 개인의 맥락을 반영할 수 있는 멀티 턴 대화 메커니즘이 필요함을 시사한다.
...
그동안의 연구들은 LLM의 성능 개선에 주로 집중해 왔으며 맥락 손실이 자주 발생하는 멀티 턴 환경에서의 개인화된 맥락 유지와 반영 개선 노력은 상대적으로 떨어졌다. 이에 본 연구에서는 **CRAG를 뼈대로, Context Engineering이 결합된 의학지식 AI Agent를 제안**하고자 한다.
...
1. **10개 노드 LangGraph 워크플로우** (구 버전: 7개 노드)
   - 추가 노드: `check_similarity`, `classify_intent`, `store_response`
   - 순환형 구조 강화: Response Cache 루프 + Self-Refine 루프
...
3. **능동적 검색 (Active Retrieval) 신규 도입**
   - 3단계 질의 분류 (rule-based → slot-based → content-based)
   - 동적 k 결정 (simple: 3, moderate: 8, complex: 15)
   - 30% 레이턴시 감소, 40% 비용 절감

### Context Engineering 강화

4. **Self-Refine 메커니즘 고도화**
...
5. **하이브리드 검색 최적화**
   - BM25 heapq 최적화: O(n log n) → O(n log k)
   - FAISS 1536차원 벡터 검색
   - RRF 융합 (k=60)
   - 정밀도 향상: 60%
...
3. **제3장**
   ...
   - 추출 프로세스 플로우차트 (MedCAT2 + 정규표현식)
   - 슬롯 기반 저장 구조 다이어그램
   - 프롬프트 조립 구조 다이어그램
   - Self-Refine 순환 구조 플로우차트
   - 10개 노드 워크플로우 다이어그램
   - 조건부 라우팅 결정 트리
   - 응답 캐시 작동 흐름도
   - Active Retrieval 3단계 분류 플로우차트
   - 하이브리드 검색 파이프라인
   - 성능 최적화 전후 비교 막대 그래프

4. **제4장**
   - 전체 시스템 아키텍처 다이어그램
   - 성능 비교 막대 그래프 (Faithfulness, Perplexity 등)
   - 실험 결과 비교 그래프
...
- 제4장 실험 설계 및 결과 (Synthea 데이터, 평가 메트릭, 통계 분석)
- 제5장 결론 및 향후 연구
- 참고문헌 (MD 파일 기반 확장)
